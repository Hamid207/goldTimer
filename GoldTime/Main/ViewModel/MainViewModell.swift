...
protocol MainViewModellProtocol {
  ...
  init(mainRouter: MainRouterProtocol?, timerTimerArray: TimerTimeArrayProtocol?, dataStore: DataStoreProtocol?, timerStatistics: TimerStatistics?, timerNotifications: TimerNotificationsProtocol?, timerDoneAlert: TimerDoneAlertProtocol?, timerAlert: TimerAlertProtocol?)
}

final class MainViewModell: MainViewModellProtocol {
  private let mainRouter: MainRouterProtocol?
  private let timerTimerArray: TimerTimeArrayProtocol?
  ...
  func timerStartStop(timerCounting: Bool, index: Int?, startTime: Date?, stopTime: Date?) {
    ...
    }else if timerCounting == false && self.index != nil {// bu deyesen alzim deyil sora yoxla
